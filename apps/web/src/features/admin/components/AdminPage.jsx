...
export default function AdminPage({ isDark }) {
  ...
  const [bulkDeptCondition, setBulkDeptCondition] = useState('person') // person | person_except | departments | departments_except | person_except_and_departments_except
  const [bulkDeptPersonId, setBulkDeptPersonId] = useState('')
  const [bulkDeptDepartmentNames, setBulkDeptDepartmentNames] = useState([]) // 複数選択（OR または 除外リスト）
  const [bulkDeptTarget, setBulkDeptTarget] = useState('') // 変更先部署
  ...
  const getBulkDeptTargetUsers = () => {
    ...
    switch (bulkDeptCondition) {
      case 'person':
        return bulkDeptPersonId ? users.filter(u => u.id === bulkDeptPersonId) : []
      case 'person_except':
        return bulkDeptPersonId ? users.filter(u => u.id !== bulkDeptPersonId) : []
      case 'departments':
        return bulkDeptDepartmentNames.length > 0
          ? users.filter(u => u.department && deptSet.has(u.department))
          : []
      case 'departments_except':
        return bulkDeptDepartmentNames.length > 0
          ? users.filter(u => !u.department || !deptSet.has(u.department))
          : []
  ...
  const handleBulkDepartmentUpdate = async () => {
    ...
    if (targetUsers.length === 0) {
      alert('条件に該当するユーザーがいません')
      return
    }
    if (!bulkDeptTarget.trim()) {
      alert('変更先の部署を選択してください')
      return
    }
    ...
    try {
      for (const u of targetUsers) {
        const { error } = await supabase
          .from('users')
          .update({ department: bulkDeptTarget.trim() })
          .eq('id', u.id)
        if (error) throw error
      }
      loadUsers()
  ...
  const toggleBulkDeptDepartment = (dept) => {
    setBulkDeptDepartmentNames(prev =>
      ...
      {activeTab === 'users' && (
        ...
        {showBulkDeptModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className={`relative w-full max-w-lg rounded-2xl shadow-xl overflow-hidden ${
              isDark ? 'bg-gray-900 border border-gray-800' : 'bg-white border border-gray-200'
            }`} onClick={e => e.stopPropagation()}>
              <div className={`px-6 py-4 border-b ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
                <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>部署を一括変更</h3>
                <p className={`text-sm mt-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                  条件に合う社員を指定した部署に変更します
                </p>
              </div>
              <div className="px-6 py-4 space-y-4">
                <div>
                  ...
                  <select
                    value={bulkDeptCondition}
                    onChange={(e) => setBulkDeptCondition(e.target.value)}
                ...
                {(bulkDeptCondition === 'person' || bulkDeptCondition === 'person_except' || bulkDeptCondition === 'person_except_and_departments_except') && (
                  <div>
                    ...
                    <select
                      value={bulkDeptPersonId}
                      onChange={(e) => setBulkDeptPersonId(e.target.value)}
                      className={`w-full px-3 py-2 rounded-xl text-sm ${
                        isDark ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white text-gray-900 border border-gray-300'
                      } focus:outline-none focus:ring-2 focus:ring-emerald-500/30`}
                    >
                      ...
                      {users.map(u => (
                        <option key={u.id} value={u.id}>{u.name || u.email} ({u.department || '未設定'})</option>
                      ))}
                    </select>
                  </div>
                )}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {[...new Set(users.map(u => u.department).filter(Boolean))].sort().map(dept => (
                        <button
                          key={dept}
                          type="button"
                          onClick={() => toggleBulkDeptDepartment(dept)}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="例: 営業部"
                    list="bulk-dept-target-list"
                    className={`w-full px-3 py-2 rounded-xl text-sm ${
                      isDark ? 'bg-gray-800 text-white border border-gray-700 placeholder:text-gray-500' : 'bg-white text-gray-900 border border-gray-300 placeholder:text-gray-400'
                    } focus:outline-none focus:ring-2 focus:ring-emerald-500/30`}
                  />
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    type="button"
                    onClick={() => !bulkDeptSaving && setShowBulkDeptModal(false)}
